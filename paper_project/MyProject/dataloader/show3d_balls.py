...
if __name__ == '__main__':
    ...
    cmap = np.array([[1.00000000e+00, 0.00000000e+00, 0.00000000e+00],
                     ...
                     [1.00000000e+00, 0.00000000e+00, 9.37500000e-02],
                     [1.00000000e+00, 0.00000000e+00, 9.37500000e-02],
                     [1.00000000e+00, 0.00000000e+00, 9.37500000e-02]]) #这个是控制颜色的
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
    ROOT_DIR = os.path.dirname(BASE_DIR)
    sys.path.append(BASE_DIR)
    sys.path.append(os.path.join(ROOT_DIR, 'data_utils'))

    from pointloader import PartNormalDataset
    root = 'D:/Dataset/shapenetcore_partanno_segmentation_benchmark_v0/shapenetcore_partanno_segmentation_benchmark_v0'
    dataset = PartNormalDataset(root = root,npoints=2048, split='test', normal_channel=False)
    idx = np.random.randint(0, len(dataset))
    data = dataset[idx] # 随机找一个点云
    point_set, _, seg = data
    # print(point_set.shape)
    # print(seg.shape)
    choice = np.random.choice(point_set.shape[0], opt.npoints, replace=True) # 采样的点
    # print("choice",choice)
    ...
    showpoints(point_set, gt, c_pred=pred, waittime=0, showrot=False, magnifyBlue=0, freezerot=False,
               background=(255, 255, 255), normalizecolor=True, ballradius=opt.ballradius) #gt pre是色彩控制 直接置为None不影响
